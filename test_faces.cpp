  ...
  while (args.Next()) {
    if (args.LastError() == SO_SUCCESS) {
      if (args.OptionId() == OPT_INTERVAL) {
        ...
        opts.threshold = atof(args.OptionArg());
      }
...
int main(int argc, char ** argv)
{
  ...
  if (!opts.result.empty()) {
    out.open(opts.result.c_str(), ios::binary);
  ...
  //   HOGPyramid pyramid(image, opts.padding, opts.padding, opts.interval);
  ...
  //   if (!Patchwork::InitFFTW((pyramid.levels()[0].rows() - opts.padding + 15) & ~15,
  //                (pyramid.levels()[0].cols() - opts.padding + 15) & ~15)) {
  ...
  //   detect(mixture, image.width(), image.height(), pyramid, opts.threshold, opts.overlap, file, out,
  //        opts.images, detections);
  ...
  { // ".txt"
    in.close();
    in.open(gt_anno_file.c_str());

    if (!in.is_open()) {
      showUsage();
      cerr << "\nInvalid image_set file " << opts.files[0] << endl;
      return -1;
    }
...
#pragma omp parallel for private(i)
    for (i = 0; i < scenes.size(); ++i) {
      ...
      for (int j = 0; j < scenes[i].objects().size(); ++j) {
        if (scenes[i].objects()[j].name() == opts.name) {
...
#pragma omp critical
      {
        for (int j = 0; j < detections.size(); ++j) {
          ...
          for (int k = 0, l = 0; k < scenes[i].objects().size(); ++k) {
            if (scenes[i].objects()[k].name() == opts.name) {
              double score = 0.0;
